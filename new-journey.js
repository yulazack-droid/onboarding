...
const $=id=>document.getElementById(id);const requestedRoleId=new URLSearchParams(location.search).get("role");let currentUser=null;
...
onAuthStateChanged(auth,user=>{if(!allowed(user)){$("loginGate").hidden=false;return}currentUser=user;$("loginGate").hidden=true;$("app").hidden=false;$("userName").textContent=user.displayName||"Scopely user";$("userEmail").textContent=user.email||"";render();const requested=roles.find(role=>role.id===requestedRoleId&&role.status==="Ready");if(requested)openRole(requested)});
